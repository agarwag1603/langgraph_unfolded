...
class MessageClassifier(BaseModel):
    MessageClassifier: Literal["Physics","Maths","Chemistry","General"] = Field(description="This is a message classfier. 'General' if not related to any known subject.")
...
def general_chatbot(state: Chatbot) -> Chatbot:
   
    prompt= f"You are a general chatbot who replies to general queries from the user: \n {state['messages']}."
    
    response = gpt_llm.invoke(prompt)

    return {'messages':[response]}

def messageclassifier_check(state: Chatbot) -> Literal["physics_chatbot","math_chatbot","chemistry_chatbot","general_chatbot"]:
    if state['MessageClassifier']=="Physics":
        return "physics_chatbot"
    elif state['MessageClassifier']=="Maths":
        return "math_chatbot"
    elif state['MessageClassifier']=="Chemistry":
...
graph.add_edge("general_chatbot", END)
